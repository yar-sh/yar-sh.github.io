...
function loadContent(nameOfPage, tabNumber, calledFromOtherHTML = false) {
 ...
 window.parent.$("body").promise().done(function(){
  ...
  if(parent.document.opened != window.location.href.split("#")[0] + nameOfPage +'.html') {
   ...
   //loads new content
   var contentXRH= new XMLHttpRequest();
   contentXRH.open('GET', '/mainInfo/' + nameOfPage+'.html', true);
   contentXRH.onreadystatechange = function() {
    if (contentXRH.readyState == 4 && contentXRH.status == 200) {
     parent.document.getElementById("contentWrap").innerHTML= this.responseText;
